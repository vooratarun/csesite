...
$count=0;
$ncount=0;
$result=mysql_query("SELECT * FROM notices WHERE active='0' ORDER BY time DESC");
...
if($notice_count>0)
{
	...
	while($row=mysql_fetch_array($result))
	{
		...
		if($count>$bnlimit && $count<=$nlimit)
		{
			...
			echo "<td style='text-align:center;width:15%;cursor:pointer;'><span style='color: #CC4852;' onclick=\"restorenotice('".$row['id']."')\">Restore</span></td>";
			echo "</tr>";
			$ncount++;
		}
		else
	...
	for($i=1;$i<=$pages;$i++)
	{
		...
		{
			echo "<a onclick=\"loadnexttrash('".$i."');\" class='inactive'>".$i."</a>";
		}
	}
	echo "</div>";
}
